@test-api @api-dashboard @api-get-gridview-item
Feature: API_Dashboard GET /api/grid-view/item
    ...
    @TC_GVI001
    Scenario Outline: TC_GVI001 - Verify user <email> could call this API to get grid view item by using company key and company type
        Given User sets GET api endpoint to get information of a company belongs to <email> using company key <companyKey>
        And  User sets GET api endpoint to get grid view item
        But User sets valid cookie of <email> and valid companyKey and valid companyType in the header
        When User sends a GET request to get grid view item of <email> by company key and company type
        Then The expected status code should be <expectedStatus>
        And Check grid view Item exist in the company, if it does not exist will create grid view Item
        And User picks random grid view of item in above response
    ...
    Scenario Outline: TC_GVI003 - Verify user <userA> could not call this API to get grid view in the item of company which does not belongs to her
        ...
        When User sends a GET request to get grid view item of <userB> by company key and company type
        Then The expected status code should be <expectedStatus>
        And The status text is "<expectedStatusText>"
        Examples:
