...
Feature: API_Admin GET/User
    ...
    Scenario Outline: USER001 - Verify <email> could call this API to get all user
        ...
        And User sets GET api endpoint to get <limitRow> users has just created
        And In Header of the request, user sets param Cookie as valid connect.sid
        When user sends a GET request to get <limitRow> latest users
        ...
        Examples:
            | user  | email              | password  | limitRow | expectedStatus |
            | admin | may27pre@gmail.com | Test1111! | 20       | 200            |
    # BUG_API
    ...
    Scenario Outline: USER002 - Verify <email> can't call this API to get all user
        ...
        Examples:
            | user | email               | password  | limitRow | expectedStatus | expectedStatusText |
            | user | may27user@gmail.com | Test1111! | 5        | 401            | Unauthorized       |

    @USER003 @bug-permission
    Scenario Outline: USER003 - Verify error when user sends this API with <cookie> cookie
        Given User sets GET api endpoint to get <limitRow> users has just created
        But User sets Cookie in HEADER as <cookie>
        When User sends a GET request to get <limitRow> latest users
        Then The expected status code should be <expectedStatus>
        And The status text is "<expectedStatusText>"
        Examples:
            | cookie  | limitRow | expectedStatus | expectedStatusText |
            | empty   | 5        | 401            | Unauthorized       |
            | invalid | 5        | 401            | Unauthorized       |