...
Then(`{} sets GET api method to get Items belonged to {} with field: {} direction: {} contain value: {}`, async function (actor, supplierOption, field, direction, value: string) {
    link = encodeURI(`${Links.API_GET_RESTOCK_SUGGESTION}?offset=0&limit=5&sort=[{"field":"${field}","direction":"${direction}"}]&where={"logic":"and","filters":[{"logic":"or","filters":[{"filters":[{"field":"flag","operator":"eq","value":"RED"},{"field":"flag","operator":"eq","value":"YELLOW"},{"field":"flag","operator":"eq","value":"ORANGE"},{"field":"flag","operator":"eq","value":"TEAL"},{"field":"flag","operator":"eq","value":"GREEN"}],"logic":"or"},{"filters":[{"field":"status","operator":"eq","value":"WATCH"}],"logic":"or"}],"currentSupplierFilters":[{"text":"[${supplierOption}]","value":"[${supplierOption}]"}]},{"logic":"or","filters":[{"field":"sku","operator":"contains","value":"${value}"},{"field":"productName","operator":"contains","value":"${value}"},{"field":"category","operator":"contains","value":"${value}"},{"field":"supplier","operator":"contains","value":"${value}"},{"field":"supplierSku","operator":"contains","value":"${value}"},{"field":"asin","operator":"contains","value":"${value}"}]},{"logic":"and","filters":[]},{"logic":"and","filters":[{"field":"status","operator":"neq","value":"IGNORE"}]},{"logic":"and","filters":[{"field":"status","operator":"neq","value":"INACTIVE"}]}]}`);
});
