...
Then(`{} sends a GET request to get grid view custom all available items`, async function (actor: string) {
    ...
    this.getGridViewCustomAllAvailaleItemsResponse = this.response = await gridViewRequest.getGridViewCustomItems(this.request, link, options);
    const responseBodyText = await this.getGridViewCustomAllAvailaleItemsResponse.text();
    if (this.getGridViewCustomAllAvailaleItemsResponse.status() == 200 && !responseBodyText.includes('<!doctype html>')) {
        this.responseBody = this.getGridViewCustomAllAvailaleItemsResponseBody = JSON.parse(await this.getGridViewCustomAllAvailaleItemsResponse.text());
        logger.log('info', `Response GET ${link}` + JSON.stringify(this.getGridViewCustomAllAvailaleItemsResponseBody, undefined, 4));
        this.attach(`Response GET ${link}` + JSON.stringify(this.getGridViewCustomAllAvailaleItemsResponseBody, undefined, 4))
    }
...
Then('Check grid view custom all available items exist in the company, if it does not exist will create grid view custom all available items', async function () {
    if (this.getGridViewCustomAllAvailaleItemsResponseBody.length < 1) {
        payload.gridState = {
            columns: [
                {
                    visible: true,
        ...
        this.createGridViewCustomAllAvaibleItemsResponse = await gridViewRequest.createGridView(this.request, Links.API_CREATE_GRIDVIEW, payload, this.headers);
        this.createGridViewCustomAllAvaibleItemsResponseBody = JSON.parse(await this.createGridViewCustomAllAvaibleItemsResponse.text())
        //logger.log('info', `Response after create ${link}` + JSON.stringify(this.createGridViewCustomAllAvaibleItemsResponseBody, undefined, 4));
        this.attach(`Response after create ${link}` + JSON.stringify(this.createGridViewCustomAllAvaibleItemsResponseBody, undefined, 4))
        // Get list after create grid view custom all available items new
        const options = {
            headers: this.headers
        }
        this.getGridViewCustomAllAvailaleItemsResponse = await gridViewRequest.getGridViewCustomItems(this.request, link, options);
        if (this.getGridViewCustomAllAvailaleItemsResponse.status() == 200) {
            this.getGridViewCustomAllAvailaleItemsResponseBody = JSON.parse(await this.getGridViewCustomAllAvailaleItemsResponse.text());
            logger.log('info', `Response GET ${link}` + JSON.stringify(this.getGridViewCustomAllAvailaleItemsResponseBody, undefined, 4));
            this.attach(`Response GET ${link}` + JSON.stringify(this.getGridViewCustomAllAvailaleItemsResponseBody, undefined, 4))
        }
    }
...
Then('{} picks random grid view of custom all available items in above response', async function (actor: string) {
    this.responseBodyOfAGridViewObject = await this.getGridViewCustomAllAvailaleItemsResponseBody[Math.floor(Math.random() * this.getGridViewCustomAllAvailaleItemsResponseBody.length)];
