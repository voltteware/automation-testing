...
function generateReport(options) {
  ...
  if (suite.totalTime / 60000000000 >= 1) {
    //Converts the duration from nanoseconds to seconds and minutes (if any)
    suite.totalTime = Math.trunc(suite.totalTime / 60000000000) + "m " + _.round((suite.totalTime % 60000000000) / 1000000000) + "s";
  } else if (suite.totalTime / 60000000000 < 1) {
    //display only seconds, if the test ran for less than a minute
    suite.totalTime = _.round(suite.totalTime / 1000000000) + "s";
  }
  ...
  function _parseFeatures(suite) {
    suite.features.forEach((feature) => {
      ...
      if (feature.duration) {
        ...
        suite.totalTime += parseInt(feature.duration);
      }
  ...
  function _createFeaturesOverviewIndexPage(suite) {
    ...
    fs.writeFileSync(
      ...
      _.template(_readTemplateFile(FEATURES_OVERVIEW_INDEX_TEMPLATE))({
        ...
        pageTitle: pageTitle,
        reportName: `Total Duration: ${suite.totalTime}`,
